...
execute as @a run scoreboard players operation temp temp2 > @s food_regen

execute as @a if score @s food_regen = temp temp2 run tag @s add win

scoreboard objectives remove temp2


#[任務]@s的回復飽食度:@s->food_regen
execute as @a run tellraw @a ["",{"text":"[\u4efb\u52d9]","color":"aqua"},{"selector":"@s","color":"dark_green"},{"text":"\u7684\u56de\u5fa9\u98fd\u98df\u5ea6:","color":"white"},{"score":{"name":"@s","objective":"food_regen"},"color":"white"}]
