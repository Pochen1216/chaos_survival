...
tellraw @a [{"text":"[系統]","color":"light_purple"},{text:"探索到了落石!小心頭頂!",color:white}]
...
scoreboard players add rock temp100 1
execute if score rock temp100 matches 2.. run tellraw @a [{"text":"[系統]","color":"light_purple"},{text:"所有玩家被落石砸死，現在開始結算....",color:white}]
execute if score rock temp100 matches 1 as @a[team=100_g] at @s run fill ~1 ~2 ~1 ~-1 ~4 ~-1 gravel
execute if score rock temp100 matches 2 as @a[team=100_g] at @s run damage @s 100 falling_stalactite
execute if score rock temp100 matches 2.. run scoreboard players set @a[team=100_g] m100_pl_jam 0
execute if score rock temp100 matches 2.. run schedule function main:misson/91_100/100/end 60t
execute if score rock temp100 matches 2.. run return 0
