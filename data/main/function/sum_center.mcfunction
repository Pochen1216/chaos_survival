...
execute at @s run summon armor_stand ~ ~ ~ {ShowArms:1b,Invisible:1b,NoBasePlate:1b,DisabledSlots:0,CustomName:[{text:"地圖中心點",bold:true,color:gold}],Marker:true,CustomNameVisible:1b,Health:1,Invulnerable:1b,NoAI:1b,NoGravity:1b,Silent:1b,Tags:["a_cen"],attributes:[{id:max_health,base:1f}]}
#execute at @s run summon armor_stand ~ ~ ~ {CustomName:[{text:"地圖中心點",bold:true,color:gold}],Marker:true}
...
execute as @e[type=minecraft:marker,limit=1,tag=m_cen] at @s run place template main:lobby ~-5 ~10 ~-5
execute as @e[type=minecraft:marker,limit=1,tag=m_cen] at @s run tp @a ~ ~12 ~
